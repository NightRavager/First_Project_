...
class TestAccountCreate:
    def test_account_create(self, db_session: Session,
        ...
        assert account_from_db is not None, (
            f"Кредит с id={response.creditId} не найден в БД"
        )
    ...
    def test_account_create_credit(self, db_session: Session,
                                   api_manager: ApiManager,
                                   create_user_credit_request: CreateUserRequest):
        response = api_manager.user_credit_steps.create_account_credit(create_user_credit_request)
